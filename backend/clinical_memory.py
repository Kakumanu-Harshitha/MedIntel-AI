...
class StateManager:
    ...
    async def contextual_update(self, current_state: ClinicalState, message: str, last_question: str = None, recent_history: Optional[List[str]] = None) -> ClinicalState:
        ...
        prompt = f"""
You are a medical conversation state processor.

Your job is to update the structured patient state using:
1. Previous structured state (source of truth)
2. Last assistant question (intent anchor)
3. Recent conversation history (last 3–4 messages)
...
1. NEVER treat CURRENT_MESSAGE as a new independent query.
2. ALWAYS interpret it in context of LAST_QUESTION and RECENT_HISTORY.
3. PREVIOUS_STATE is the source of truth — NEVER remove existing data.

-------------------------------------

CONTEXT INTERPRETATION RULES:

1. If message is short (e.g., "3 days", "no", "yes"):
   → It is an ANSWER to the LAST_QUESTION

2. If LAST_QUESTION asked about duration:
   - "3 days", "2 weeks", "since yesterday"
   → update duration

3. If LAST_QUESTION asked about additional symptoms:
   - "no" → do NOT add symptoms
   - "yes cough" → add "cough"
   - "cough" → treat as additional symptom

4. If message contains BOTH:
   - "3 days and cough"
   → update BOTH duration and symptoms

5. If message is like "3 days no":
   → duration = "3 days"
   → no new symptoms
   → KEEP previous symptoms

-------------------------------------

SYMPTOM HANDLING RULES:

1. Extract ALL symptoms mentioned (not just one)
2. Normalize variations:
   - "head ache" → "headache"
   - "vomtings" → "vomiting"
   - "blurry vision" → "blurred vision"
3. DO NOT remove previous symptoms
4. Avoid duplicates
...
- DO NOT say "unclear"
- DO NOT reset state
- DO NOT ignore previous symptoms
- DO NOT overwrite symptoms with empty list

-------------------------------------

IMPORTANT:

If CURRENT_MESSAGE does not contain new symptoms:
→ KEEP previous symptoms unchanged

If CURRENT_MESSAGE does not contain duration:
→ KEEP previous duration unchanged
...
PREVIOUS_STATE:
{{ "symptoms": ["stomach pain", "vomiting"], "duration": null }}

LAST_QUESTION:
"How long have you been experiencing this? Any other symptoms?"
